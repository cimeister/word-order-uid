...
positions <- c(
       "Real", "Reverse", "Sort-Freq", "Sort-Freq-Rev", "Approx", "Min-DL-Loc",
       "Min-DL-Opt", "Efficient-OV", "Efficient-VO", "Random-1", "Random-2", "Random-3", "Random-4", "Random-5"
)
positions_new <- c(
       "Real", "Reverse", "Sort_Freq", "Sort_Freq_Rev", "Approx", "Min_DL_Loc",
       "Min_DL_Opt", "Efficient_OV", "Efficient_VO", "Random_1", "Random_2", "Random_3", "Random_4", "Random_5"
)
language_order <- c(
       "English (SVO)", "Farsi (SOV)", "French (SVO)", "German (SVO)",
       "Hindi (SOV)", "Hungarian (SVO)", "Indonesian (SVO)", "Russian (SVO)", "Turkish (SOV)", "Vietnamese (SVO)"
)
...
make_point_plot <- function(data, title, imgname, value, error, ncol) {
       ...
       data$language <- recode_factor(data$language,
              en = "English (SVO)", de = "German (SVO)", ru = "Russian (SVO)", vi = "Vietnamese (SVO)",
              hi = "Hindi (SOV)", fa = "Farsi (SOV)", fr = "French (SVO)", id = "Indonesian (SVO)", tr = "Turkish (SOV)", hu = "Hungarian (SVO)"
       )
       data$variant <- recode_factor(data$variant,
              Sort_Freq = "Sort-Freq",
              Sort_Freq_Rev = "Sort-Freq-Rev",
              Min_DL_Loc = "Min-DL-Loc",
       ...
       plot <- data %>%
              ...
              mutate(
                     variant = factor(variant, levels = positions),
              ...
              scale_y_continuous(breaks = scales::pretty_breaks(n = 3, high.u.bias = 2.0)) +
...
ggsave(
       filename = paste("joint_surprisal_and_variance", ".png", sep = ""),
       path = paste(DATA_DIR, "plots", sep = "/"),
       width = 12.75, height = 7
)
ggsave(
       filename = paste("joint_surprisal_and_variance", ".svg", sep = ""),
       path = paste(DATA_DIR, "plots", sep = "/"),
       width = 12.75, height = 7
)

surprisal.cc <- make_point_plot(data.cc, "Mean Test\nSurprisal", "surprisal", "surprisal_mean", "surprisal_sem", 1)
variance.cc <- make_point_plot(data.cc, "Mean Surprisal\nVariance", "surprisal_variance_cc100", "surprisal_var_mean", "surprisal_var_sem", 1)

arranged <- ggarrange(surprisal.cc, variance.cc,
       labels = c("", ""),
       ncol = 2, common.legend = TRUE,
       legend = "right", align = "hv"
)

annotate_figure(arranged, top = text_grob("CC100 Dataset Results", size = 34, family = "serif"))

ggsave(
       filename = paste("suprisal_variance_cc100", ".png", sep = ""),
...
ggsave(
       filename = paste("joint_mean_regress_and_uid_power", ".png", sep = ""),
       path = paste(DATA_DIR, "plots", sep = "/"),
       width = 12.75, height = 7
)
ggsave(
       filename = paste("joint_mean_regress_and_uid_power", ".svg", sep = ""),
       path = paste(DATA_DIR, "plots", sep = "/"),
       width = 12.75, height = 7
)
...
ggsave(
       filename = paste("joint_doc_initial_and_uid_loc", ".png", sep = ""),
       path = paste(DATA_DIR, "plots", sep = "/"),
       width = 12.75, height = 7
)
ggsave(
       filename = paste("joint_doc_initial_and_uid_loc", ".svg", sep = ""),
       path = paste(DATA_DIR, "plots", sep = "/"),
       width = 12.75, height = 7
)
